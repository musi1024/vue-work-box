...
const getFlag = () => flag++;
const getUrlOrigin = url => /^https?:\/\/[\w-.]+(:\d+)?/i.exec(url)[0];
const returnMock = ({ success, data, error }) =>
  success ? Promise.resolve(data) : Promise.reject(error);
class HealthStarSdk {
  ...
  _init(mode = 'production', mockDatas = {}) {
    ...
    const origin = getUrlOrigin(IFRAME_URL_MAP[mode]);
    initMessagePromise = this._initMessageList(origin);
    MOCK = mockDatas;
  }
  _initMessageList(origin) {
      ...
      window.addEventListener(
        ...
        e => {
          ...
          if (ready) {
            ...
            MESSAGE_CALLBACK_LIST[flag] && MESSAGE_CALLBACK_LIST[flag](res);
  ...
  getUserId() {
    return USE_MOCK
      ? returnMock(MOCK.getUserId).then(({ model }) => model)
      : initMessagePromise;
  }

  async run(methodName, params) {
    if (USE_MOCK) return returnMock(MOCK[methodName]);
